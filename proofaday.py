...
MACROS = [
    MacroTextSpec(name, simplify_repl=repl) for name, repl in (
        ('R', '\N{DOUBLE-STRUCK CAPITAL R}'),
        ('Z', '\N{DOUBLE-STRUCK CAPITAL Z}'),
        ('N', '\N{DOUBLE-STRUCK CAPITAL N}'),
        ('lfloor', '\N{LEFT FLOOR}'), ('rfloor', '\N{RIGHT FLOOR}'),
        ('lceil', '\N{LEFT CEILING}'), ('rceil', '\N{RIGHT CEILING}'),
...
if __name__ == '__main__':
    ...
    if ret is not None:
        ...
        theorem, proof = L2T.latex_to_text(theorem, latex_context=ARGS), L2T.latex_to_text(proof,latex_context=ARGS)
